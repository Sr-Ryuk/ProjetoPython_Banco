...
def efetuar_transferênia() -> None:
    if len(contas) > 0:
        numero: int = int(input('Informe o numero da sua conta: '))

        conta_o: Conta = buscar_conta_por_numero(numero)

        if conta_o:
            numero_d: int = int(input('Digite o numero da conta destino: '))

            conta_d: Conta = buscar_conta_por_numero(numero_d)

            if conta_d:
                valor: float = float('Digite o valor a ser transferido')

                conta_o.transferir(destino=conta_d, valor=valor)

            else:
                print(f'A conta destino com n° {numero_d} nãao foi econtrada.')
        else:
            print(f'Não econtramos a conta {numero}')
    else:
        print('Ainda não existem contas cadastradas.')
        sleep(2)
        menu()
...
def listar_contas() -> None:
    if len(contas) > 0:
        ...
        print('Ainda não possui contas cadastradas.')
        sleep(2)
        menu()  


def buscar_conta_por_numero(codigo: int) -> Conta:
    c: Conta = None
